document.addEventListener("DOMContentLoaded", () => {
  ...
  const initActiveNavLinks = () => {
    ...
    navLinks.forEach((link) => {
      ...
      } else if (!isLinkToHome && currentPath.includes(linkPath.substring(1))) {
        // substring(1)で先頭の'/'を除く
        link.classList.add("nav-active");
      }
    });
  };
  ...
  const initGalleryToggle = () => {
    document.body.addEventListener("click", (e) => {
      ...
      const galleryWrapper = portfolioCard.querySelector(
  ...
  const initFilter = () => {
    const filterContainer = document.querySelector(
      ".portfolio-categories, #level-filters"
    );
    ...
    const applyFilters = () => {
      items.forEach((item) => {
        ...
        const categoryMatch =
          currentCategory === "all" || itemCategories.includes(currentCategory);
    ...
    document.body.addEventListener("click", (e) => {
      ...
      if (parentId === "category-filters") {
        ...
        document
          .querySelectorAll("#category-filters .category-btn")
          .forEach((btn) => btn.classList.remove("active"));
        const levelAllBtn = document.querySelector(
          '#level-filters [data-level="all"]'
        );
        if (levelAllBtn) levelAllBtn.click();
      } else if (parentId === "level-filters") {
        currentLevel = button.dataset.level;
        document
          .querySelectorAll("#level-filters .category-btn")
          .forEach((btn) => btn.classList.remove("active"));
        const categoryAllBtn = document.querySelector(
  ...
  const initSiteWideSpeech = () => {
    setTimeout(() => {
      if (
        typeof SiteWideSpeaker === "undefined" ||
        !window.siteSpeaker ||
        window.siteSpeaker.isLoading
      ) {
      ...
      readableElements.forEach((el) => {
        ...
        if (
          el.parentElement.closest(".readable") &&
          !el.classList.contains("readable")
        ) {
          return;
        }
        ...
        if (
          el.textContent.trim() === "" ||
          el.classList.contains("post-card-category") ||
          el.classList.contains("article-meta") ||
          el.querySelector(".speech-button")
        ) {
          return;
        }
        ...
        playBtn.addEventListener("click", (e) => {
          ...
          if (el.dataset.speech) {
            ...
            const buttonToRemove =
              elementForSpeech.querySelector(".speech-button");
            if (buttonToRemove) buttonToRemove.remove();

            const speechSpans =
              elementForSpeech.querySelectorAll("[data-speech]");
  ...
  const initImageSpeech = () => {
    setTimeout(() => {
      ...
      const imageContainers = document.querySelectorAll(
        ".article-content, .lesson-content, .novel-content"
      );
